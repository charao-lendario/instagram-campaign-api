...
- [x] AC1: Given the `/posts` page loads with data, when `usePosts()` returns post data, then a shadcn `Table` renders with columns: Candidato, Data, Legenda (caption), Curtidas, Comentarios, % Positivo, % Negativo, Sentimento Medio.
- [x] AC2: Given the post table, when the page first loads, then rows are sorted by comment count descending (default sort per FR-010).
- [x] AC3: Given the column headers, when a user clicks any column header, then the sort order toggles between ascending and descending for that column, and the active sort column is visually indicated (arrow icon up/down).
- [x] AC4: Given the sort state changes, when a column is sorted, then `usePosts()` is called with the updated `sort_by` and `order` parameters, fetching sorted data from the API.
- [x] AC5: Given the `CandidateFilter` in the header, when a single candidate is selected, then the table shows only that candidate's posts (`candidate_id` param sent to the API).
- [x] AC6: Given the post table, when there are more than 20 rows, then a "Carregar mais" button appears below the table. When clicked, the next 20 posts are appended to the table (using the `offset` parameter).
- [x] AC7: Given a post row, when the caption text exceeds 80 characters, then it is truncated with "..." and the full caption is shown in a `title` attribute (HTML tooltip) on hover.
- [x] AC8: Given the `/comparison` page loads, when `useComparison()` returns data, then a 2-column layout renders side by side with one column per candidate, each showing: display name, total posts, total comments, average sentiment score (with SentimentBadge), total engagement, sentiment distribution bars (from Story 2.3 components), and top 3 themes.
- [x] AC9: Given the comparison page, when rendered, then a trend direction indicator shows for each candidate: an upward green arrow with "Melhorando" when `trend.direction === "improving"`, a downward red arrow with "Declinando" when `"declining"`, and a horizontal gray dash with "Estavel" when `"stable"`.
- [x] AC10: Given the comparison page, when rendered, then a mini sparkline Recharts `LineChart` without axes or labels shows the last visible data points per candidate (from `trend.recent_avg` and `trend.previous_avg` as 2 data points, or use the timeline endpoint for the last 10 posts if feasible within this story scope).
- [x] AC11: Given data is loading on either page, when hooks return `loading: true`, then skeleton placeholders render for the table rows (posts page) or comparison cards (comparison page).
- [x] AC12: Given an API error on either page, when hooks return `error`, then the shared `ErrorMessage` component renders with a retry button.
...
- [x] T1: Create TrendIndicator component (AC9)
  - [x] T1.1: `pnpm add lucide-react` (if not installed)
  - [x] T1.2: Create `src/components/dashboard/trend-indicator.tsx`

- [x] T2: Create Sparkline component (AC10)
  - [x] T2.1: Create `src/components/charts/sparkline.tsx` using 2-point Recharts LineChart
  - [x] T2.2: Verify rendering with minimal configuration (no axes/grid)

- [x] T3: Build posts page with sortable table (AC1, AC2, AC3, AC4, AC5, AC6, AC7)
  - [x] T3.1: Convert `/posts` placeholder to Client Component
  - [x] T3.2: Implement sort state and column header click handlers
  - [x] T3.3: Wire `usePosts` hook with sort and candidate params
  - [x] T3.4: Build shadcn Table with all 8 columns
  - [x] T3.5: Implement caption truncation (80 chars + title attribute)
  - [x] T3.6: Implement "Carregar mais" button with offset pagination
  - [x] T3.7: Implement loading (table row skeletons) and error states

- [x] T4: Build comparison page (AC8, AC9, AC10, AC11, AC12)
  - [x] T4.1: Convert `/comparison` placeholder to Client Component
  - [x] T4.2: Wire `useComparison` hook
  - [x] T4.3: Build 2-column candidate card layout
  - [x] T4.4: Integrate reused components: SentimentBadge, SentimentBar (from Story 2.3)
  - [x] T4.5: Add TrendIndicator and Sparkline to each card
  - [x] T4.6: Display top 3 themes with formatted labels and count badges
  - [x] T4.7: Implement loading (card skeletons) and error states
...
|------|-------------|--------|------------|
| Sort state and pagination offset desync on filter change | Medium | Medium | Reset offset to 0 whenever sort column, sort order, or candidate filter changes. Technical notes already specify this pattern. |
| "Carregar mais" appending duplicates if API data changes between fetches | Low | Medium | API uses deterministic sort + offset pagination. Risk is minimal for static analysis data. Accept for MVP. |
| Sparkline with only 2 data points looks minimal/uninformative | Medium | Low | Acceptable for MVP per AUTO-DECISION. The trend direction indicator provides the primary visual cue; sparkline supplements it. |
| Large story scope (2 full pages, 12 ACs) increases implementation risk | Medium | Medium | Tasks are well-decomposed (T1-T4 with subtasks). Posts page and comparison page are independent after shared components (T1, T2) are built. |
| Caption truncation at 80 chars may cut mid-word | Low | Low | Use CSS `text-overflow: ellipsis` or substring cut. Full text available on hover via `title` attribute. |
...
| **Notes** | Largest story in Epic 2 (8 points). Well-decomposed tasks. Both API endpoints match architecture.md. Component reuse from Story 2.3 is correctly identified. Sparkline AUTO-DECISION is reasonable for MVP. |
...
|------|--------|-------------|
| `src/components/dashboard/trend-indicator.tsx` | Created | TrendIndicator with Lucide icons and direction colors |
| `src/components/charts/sparkline.tsx` | Created | Sparkline mini LineChart (2 data points, no axes) |
| `src/app/posts/page.tsx` | Modified | Server Component wrapper with Suspense boundary |
| `src/app/posts/posts-content.tsx` | Created | Client Component with sortable table, load more pagination |
| `src/app/comparison/page.tsx` | Modified | Server Component wrapper with Suspense boundary |
| `src/app/comparison/comparison-content.tsx` | Created | Client Component with candidate cards, trends, sparklines |
