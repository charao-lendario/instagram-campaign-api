...
- [ ] AC1: Given the `/insights` page loads, when data is available (suggestions fetched on page mount), then 3 to 5 suggestion cards are rendered, each in a shadcn `Card` with a distinctive left-side colored border indicating priority.
- [ ] AC2: Given each suggestion card, when rendered, then it displays: title (bold, larger text), description (paragraph), supporting_data text (in a visually distinct `<blockquote>` or highlighted box with an info icon), and a priority `Badge` (red "Alta" for `high`, yellow "Media" for `medium`, green "Baixa" for `low`).
- [ ] AC3: Given the priority border, when a card is rendered with `priority === "high"`, then the left border is red (`border-l-4 border-red-500`); `"medium"` renders yellow (`border-l-4 border-yellow-500`); `"low"` renders green (`border-l-4 border-green-500`).
- [ ] AC4: Given a "Atualizar sugestoes" button at the top of the page, when clicked, then `POST /api/v1/analytics/suggestions` is called with the current `candidate_id` filter (or no body if "all" is selected), and the cards re-render with the new suggestions.
- [ ] AC5: Given the "Atualizar sugestoes" button is clicked, when the LLM call is in progress (may take 3-10 seconds), then the button is disabled, shows a spinning `Loader2` icon from lucide-react, and the cards area shows a loading overlay or skeleton.
- [ ] AC6: Given the `CandidateFilter` in the header, when a specific candidate is selected, then the `candidate_id` is included in the suggestions request body on the next refresh. On initial page load, the current filter value is used.
- [ ] AC7: Given a disclaimer below the suggestion cards, when rendered, then the text "Sugestoes geradas por IA com base nos dados disponiveis. Valide com a equipe de campanha antes de agir." is visible in small gray text.
- [ ] AC8: Given data is loading on page mount, when `loading: true`, then 3-5 skeleton cards with the shape of suggestion cards render as placeholders.
- [ ] AC9: Given an API error (including LLM provider failure), when the suggestions request fails, then the shared `ErrorMessage` component renders with the message "Nao foi possivel gerar sugestoes. Tente novamente." and a retry button.
- [ ] AC10: Given no data exists yet (API returns empty suggestions array or error due to insufficient data), when the empty state renders, then `EmptyState` shows "Dados insuficientes para gerar sugestoes. Execute uma coleta de dados primeiro." with a button to trigger scraping.
- [ ] AC11: Given the page layout, when rendered, then the suggestions cards are ordered by priority (high first, medium second, low last) regardless of the order returned by the API.
- [ ] AC12: Given the tab navigation, when the user navigates to `/insights`, then this is the 7th and final tab ("Insights") and all previous tabs remain functional.
...
- [ ] T1: Create SuggestionCard component (AC1, AC2, AC3)
  - [ ] T1.1: Create `src/components/dashboard/suggestion-card.tsx`
  - [ ] T1.2: Implement priority border class logic
  - [ ] T1.3: Implement priority badge with localized labels (Alta/Media/Baixa)
  - [ ] T1.4: Style supporting_data as a highlighted info box with InfoIcon

- [ ] T2: Build insights page (AC1, AC4, AC5, AC6, AC7, AC8, AC9, AC10, AC11, AC12)
  - [ ] T2.1: Convert `/insights` placeholder to Client Component
  - [ ] T2.2: Wire `useSuggestions` hook for initial load
  - [ ] T2.3: Implement priority sorting on received suggestions
  - [ ] T2.4: Implement "Atualizar sugestoes" button with Loader2 spinner
  - [ ] T2.5: Add CandidateFilter integration (pass candidate_id to refresh)
  - [ ] T2.6: Add last-generated timestamp display
  - [ ] T2.7: Implement loading state (3 skeleton cards)
  - [ ] T2.8: Implement error state with retry
  - [ ] T2.9: Implement empty state with trigger scraping button
  - [ ] T2.10: Add disclaimer text at bottom

- [ ] T3: Navigation verification (AC12)
  - [ ] T3.1: Confirm "Insights" is the 7th tab in nav-tabs.tsx
  - [ ] T3.2: Confirm all 7 tabs navigate correctly with active state
  - [ ] T3.3: Run `pnpm build` to confirm zero TypeScript/lint errors across the entire project
